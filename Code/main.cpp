...
static float rot_rl,
    rot_ud; // Kretanje kamere u odnosu na svet (right-left  up-down)
...
static int
    texture_param; // Parametar koji regulise sirinu slike texture drvo-zgrada
...
static void on_timer(int v) {
  ...
  if (anim_param > rand_trn &&
      (-rot_rl < rand_rot + 25 / (float)rand_trn &&
       -rot_rl > rand_rot - 25 / (float)rand_trn) &&
      rot_ud > 0.5 / (float)rand_trn && rot_ud < 3 * 25 / (float)rand_trn) {
...
static void on_display(void) {
  ...
  glTexCoord2f(0, 0);
  glVertex3f(100, 0, -10);
  glTexCoord2f(3 * texture_param, 0);
  glVertex3f(-100, 0, -10);
  glTexCoord2f(3 * texture_param, 3 * texture_param);
  glVertex3f(-100, 0, 300);
  glTexCoord2f(0, 3 * texture_param);
  glVertex3f(100, 0, 300);
  glEnd();
  ...
  if (on_going <= 0 && on_going == 0) {
    ...
    write_text("Click 'F' for forest mode or 'C' for city mode",
               width / 2 - 160, height * 0.90);
